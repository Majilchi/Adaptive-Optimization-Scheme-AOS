...
function [Positions, SwarmBestX, SwarmBestO, FE, Beta_k]=AOS(alpha, UB, LB, Positions,...
    fitness, FunIndex, dim, Particles_no, SwarmBestX, SwarmBestO, n, m, beta, Beta_k, FE, FEmax)
Fit(:,1) = fitness;
...
%____________________Levy_______________________%
  ...
  X_Levy(1,:)=SwarmBestX.*Levy(n,m,beta)*alpha;
      ...
      if (fitness(Fit(j,2))<SwarmBestO)
          SwarmBestO = fitness(Fit(j,2));
          SwarmBestX = Positions(Fit(j,2),:);
      end
             FE = FE + 1;
...
%__________________________CLS+OBL__________________________%
           [X_CLS, Beta_k]= CLS(Beta_k, FE, FEmax, UB, LB, SwarmBestX);
           ...
           FE=FE+2;
           if(F_OBL<F_CLS && F_OBL<SwarmBestO)
               SwarmBestO=F_OBL;
               SwarmBestX=X_OBL;
           elseif (F_CLS<F_OBL && F_CLS<SwarmBestO)
               SwarmBestO=F_CLS;
               SwarmBestX=X_CLS;
           end
